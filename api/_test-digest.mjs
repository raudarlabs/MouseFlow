...
group('дашборд показывает то, что посчитал');
{
  ...
  /* ГРАФИК ПО ДНЯМ УШЁЛ СО СТРАНИЦЫ (шаг 8, 2026-09-22): он считался из `user_run`, а страница спрашивает
   * половину `did`. Вместе с ним ушёл и `dayWindow` - тот, кто ПИСАЛ в адрес сутки по UTC.
   *
   * А ЧИТАТЕЛЬ ОСТАЛСЯ, и это не забытый код: ссылки, отправленные до сегодня, несут ровно такое окно, и
   * страница обязана узнать его и показать как один день, а не как «30.08 - 31.08». Правило то же, ради
   * которого писатель резал по UTC: по UTC эти сутки и посчитаны, местная полночь вернула бы другой набор
   * прогонов. Проверяется теперь чтение - единственное, что от пары осталось. */
  check('сутки по UTC из уже отправленной ссылки узнаются как один день',
    /const wholeUtcDay = /.test(page) && /getUTCHours\(\) === 0/.test(page)
      && !/const dayWindow = /.test(page));
}
