...
export const LIMITS = {
  ...
  /* Просьба, пришедшая из мессенджера (SPLIT-PLAN §7.2, шаг 14a). Тратится РОВНО на построение плана -
   * один вызов модели перед прогоном, - а не на сам прогон: прогон уже считается ключом `step`, и считать
   * его дважды значило бы, что задача из чата исчерпывает свой потолок вдвое быстрее чужой.
   *
   * ЧИСЛО МЕНЬШЕ ОСТАЛЬНЫХ, и это не осторожность вообще, а осторожность К МЕСТУ: за сообщением в чате
   * стоит настоящая мышь, а отправитель - единственный вход, который человек не открывал сам, а которому
   * дал право один раз и забыл. Шесть планов за пять минут - это быстрее, чем можно прочитать шесть
   * планов. Незнакомцу не считается ничего, потому что для него ничего и не вызывается: до спаривания
   * модель не трогают вовсе. */
  telegram: { max: 6, windowMs: 300_000, product: 'do' },
  /* Распознавание речи (SPLIT-PLAN §7, шаг 13). Продиктованная цель - это секунды звука, и человек
   * диктует не быстрее, чем говорит: тридцать за пять минут - это непрерывная диктовка без единой паузы,
   * то есть уже не человек. Числом ближе к `claude`, чем к `telegram`, потому что стоит это заметно
   * дешевле хода модели и потому что за ним не стоит мышь - за ним стоит поле ввода. */
  transcribe: { max: 30, windowMs: 300_000, product: 'do' },
};
