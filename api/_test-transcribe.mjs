...
group('язык диктовки: «авто» у сервера, настоящий - у браузера');
{
  ...
  /* ФРАЗЫ ПРО ЗВУК В ПАНЕЛИ БОЛЬШЕ НЕТ - РЕШЕНИЕ ВЛАДЕЛЬЦА, дважды и явно (2026-09-21): окно на одну
   * фразу, и абзац занимал в нём четверть высоты. Возражение было высказано один раз и снято; записано
   * в SPLIT-PLAN §7 как отступление, чтобы следующий читатель не вернул строку молча и не решил, что
   * правило забыли.
   *
   * ЧТО ОСТАЛОСЬ, И ПОЧЕМУ ЭТОГО НЕ УБРАЛИ ЗАОДНО: подпись на кнопке микрофона. Она не занимает места -
   * то есть не спорит с причиной, по которой строку убрали, - и оставляет ответ на вопрос «куда уходит
   * мой голос» в одном наведении, а не нигде. На большой странице Create фраза стоит целиком. */
  check('обещание о звуке осталось подписью на кнопке микрофона',
    /title=\{dictation\.via === 'openai' \? WHERE_AUDIO_GOES : STAYS_HERE\}/.test(panel));
  /* ОДНА СТРОКА, И ВСЕ ТРОЕ НА НЕЙ. С `items-end` язык оказывался посередине высоты, а микрофон и
   * отправка внизу: три элемента одной строки в двух разных местах, и заметно это только на длинном
   * тексте - то есть не в тот момент, когда вёрстку писали. */
  /* СОВЕТ ПО МЕСТУ. В панели нет адресной строки, и «разрешите для этого сайта в адресной строке»
   * отправляло человека искать то, чего перед ним нет: там разрешение принадлежит ПРИЛОЖЕНИЮ и живёт
   * в System Settings. Найдено живым запуском. */
  check('отказ микрофона советует то, что есть перед человеком',
    /inPanel\(\)/.test(speech) && /System Settings → Privacy & Security → Microphone/.test(speech)
      && /Allow it for this site in the address bar/.test(speech));

  check('и кнопки выровнены по центру строки, а не по её дну',
    /flex items-center gap-1 rounded-2xl/.test(panel) && !/flex items-end gap-1 rounded-2xl/.test(panel));
  check('и строкой в панели больше не стоит',
    !/text-ink-inactive[\s\S]{0,80}WHERE_AUDIO_GOES/.test(panel));
