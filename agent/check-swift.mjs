...
group('панель нативная только снаружи, и аккорд не берёт лишнего');
{
  ...
  /* РАЗМЕР ПОД СОДЕРЖИМОЕ - и этот пункт написан после живого запуска: первое, что видит человек, это не
   * композер, а ВХОД в аккаунт, и в окно размером с подсказку он не помещается. Панель, сломанная на
   * первом же показе, - это панель, у которой не будет второго. */
  check('окно подгоняется под чужой экран, а не остаётся размером с композер',
    !!panel && /private static let askSize/.test(panel) && /private static let elseSize/.test(panel)
    && /\(url\?\.path \?\? ""\) == "\/panel"/.test(panel));
  /* НА didCommit, А НЕ НА didFinish: страница входа рисуется задолго до конца загрузки, и окно,
   * дорастающее в последний момент, человек успевает увидеть маленьким. */
  check('и подгоняется в момент показа страницы, а не в конце её загрузки',
    !!panel && /func webView\(_ webView: WKWebView, didCommit navigation/.test(panel)
    && !/didFinish[\s\S]{0,80}fit\(to:/.test(panel));
  /* И РУКА ВСЕГДА ПРАВА: окно resizable, а свой размер возвращается только если растягивали мы. */
  check('и рука сильнее догадки - окно тянется, а наш размер возвращается только поверх нашего',
