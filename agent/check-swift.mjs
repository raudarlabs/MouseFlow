...
group('панель нативная только снаружи, и аккорд не берёт лишнего');
{
  ...
  /* БЕЗ КОММЕНТАРИЕВ там, где ищут ОТСУТСТВИЕ: причины решений в них и названы - иначе следующий
   * читатель вернёт заголовок окна, не зная, чем это кончилось. Пин обязан смотреть на код, а не на
   * объяснение. Поймано дважды за десять минут, на swift и на tsx. */
  ...
  check('и перезагружается при закрытии, а не при открытии',
    /load\(URLRequest/.test(hide.slice(0, 700)));

  /* ВЫСОТУ ГОВОРИТ СТРАНИЦА - третья редакция, и каждая от живого запуска. Сначала размер был один и
   * посчитан под композер: в него не поместился экран входа. Потом их стало два, по адресу. Оба раза
   * окно ДОГАДЫВАЛОСЬ о содержимом, и выглядело это как коробка с пустотой внутри. Мерить умеет только
   * тот, кто знает, - страница. */
  check('высоту присылает страница, а окно её не угадывает',
    !!panel && /didReceive message: WKScriptMessage/.test(panel)
    && /body\["height"\] as\? Double/.test(panel));
  check('и мост принимает ровно одно сообщение, по имени',
    !!panel && /message\.name == "mouseflow"/.test(panel));
  /* И ПРОВЕРЯЕТ ЧИСЛО. Мост из веб-страницы в нативное окно - не место для доверия к присланному:
   * бесконечность или ноль превратили бы окно в точку или в экран целиком. */
  check('и число проверяется, а не принимается на слово',
    !!panel && /asked\.isFinite, asked > 0/.test(panel)
    && /max\(Panel\.minHeight, min\(asked\.rounded\(\), ceiling\)\)/.test(panel));
  /* ПОЛ И СТАРТ - РАЗНЫЕ ЧИСЛА, и это была ошибка, видимая глазом: полом служила firstHeight, то есть
   * окно не могло стать ниже своего СТАРТОВОГО размера, и под пилюлей в одну строку оставалась пустота
   * ровно в ту разницу, которую страница честно просила убрать. «Пока не знаем» и «ниже не бывает» -
   * два разных утверждения, и одно число на две роли верно в одном случае из двух. */
  check('и стартовая высота - не то же самое, что нижний предел',
    !!panel && /private static let minHeight/.test(panel) && /private static let firstHeight/.test(panel));

  /* ВВОД РАБОТАЕТ КАК ВЕЗДЕ, и обе поломки были от одного флага. Неактивирующая панель принимает
   * нажатия, не делая приложение активным, - и ломает ⌘V (команды правки разносит главное меню
   * АКТИВНОГО приложения) и повтор при удержании клавиши. Мы всё равно активируем приложение в show(). */
  check('панель не отказывается активировать приложение',
    !!panel && !/nonactivatingPanel/.test(panelCode));
  /* И ГЛАВНОЕ МЕНЮ СУЩЕСТВУЕТ РАДИ ⌘V: в macOS это не «встроенное поведение поля», а пункт Paste со
   * своим сочетанием. У accessory-приложения меню не видно - оно и есть просто таблица сочетаний. */
  check('у агента есть меню правки, иначе в панель нельзя вставить',
    /app\.mainMenu = mainMenu/.test(src)
    && /#selector\(NSText\.paste\(_:\)\), "v"/.test(src)
    && /#selector\(NSText\.selectAll\(_:\)\), "a"/.test(src));
  /* И В НЁМ ТОЛЬКО ПРАВКА: пункт, которого никто не увидит, всё равно перехватит своё сочетание у
   * приложения впереди - ⌘Q в невидимом меню это чужое приложение, закрытое нашей панелью. */
  check('и в нём нет ничего, кроме правки',
    !/mainMenu[\s\S]{0,900}?"Quit"/.test(src));
  /* И ВОЗВРАЩАЕТСЯ К ОДНОЙ СТРОКЕ на новой странице и при закрытии - иначе экран входа, уйдя, оставил
   * бы после себя окно в семьсот точек. */
  check('и новая страница начинается с одной строки',
    !!panel && /didCommit navigation[\s\S]{0,200}?resize\(to: Panel\.firstHeight\)/.test(panel));

  /* ВЫХОД ЕСТЬ, И ИХ ТРИ. Заголовка у окна больше нет - он ничего не сообщал человеку, который сам его
   * вызвал, а стоил полосы и вида «диалог». Но панель без выхода это ловушка, и с крестиком их был
   * один, а стало три: Escape, щелчок мимо окна и тот же аккорд ещё раз. */
  check('окно без заголовка и рамки',
    !!panel && /\.borderless/.test(panelCode) && !/\.titled/.test(panelCode));
  ...
  check('и щелчок мимо окна тоже',
    !!panel && /didResignKeyNotification/.test(panel));
  /* И УХОД НЕ ЗОВЁТ САМ СЕБЯ: orderOut отнимает ключевой статус, на который подписан тот же наблюдатель. */
  check('и уход не вызывает сам себя через своё же уведомление',
  ...
  /* ДВА АККОРДА: текст и «нажал и говори». Второй - не режим первого и не двойное нажатие: то и другое
   * надо помнить, а отдельная клавиша просто есть. Различаются по id внутри ОДНОГО обработчика - Carbon
   * даёт его один на приложение, и второй обработчик получал бы и чужие нажатия. */
  check('аккордов два, и различаются они по id события',
    !!hotkey && /static let saidSpeak = "⌃⌥⇧Space"/.test(hotkey)
    && /id\.id == Hotkey\.speakId/.test(hotkey)
    && (hotkey.match(/RegisterEventHotKey\(/g) || []).length === 2);
  /* И СНИМАЮТСЯ ОБА: один снятый аккорд во время записи - это дыра в записи ровно там, где нажали. */
  check('и снимаются оба, а не один',
    !!hotkey && /UnregisterEventHotKey\(have\)[\s\S]{0,120}?speakRef/.test(hotkey));
  /* ВТОРОЙ НЕ ОТКРЫВАЕТ ВТОРОЙ СТРАНИЦЫ: она уже загружена и уже та самая, а перезагрузка стоила бы
   * ровно того, ради чего панель прогревают. */
  check('и «говорить» зовёт уже открытую страницу, а не грузит вторую',
    !!panel && /window\.__mouseflowSay/.test(panelCode) && !/panel\?say/.test(panelCode));
  /* ЯЗЫК - С РАСКЛАДКИ: она говорит, на чём человек пишет СЕЙЧАС, что точнее браузерного списка
   * языков - из-за которого русская речь однажды распозналась как английская. */
  check('и язык берётся с раскладки, базовым кодом',
    !!panel && /TISCopyCurrentKeyboardInputSource/.test(panel)
    && /split\(separator: "-"\)\.first/.test(panel));

  /* И ЕГО ВИДНО. Горячая клавиша, о которой нигде не написано, - это клавиша, которой не пользуются. */
  check('аккорды написаны в пункте меню, а не только в коде',
    /Shortcuts \\\(Hotkey\.said\) · \\\(Hotkey\.saidSpeak\)/.test(src)
    && /static let said = "⌃⌥Space"/.test(src));
  check('и «сказать» тоже открывается мышью', /"Say a Task…"/.test(src));
