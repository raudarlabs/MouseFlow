...
export function PanelView() {
   ...
   * чего панель прогревают.
   *
   * ЯЗЫК ПРИЕЗЖАЕТ С РАСКЛАДКИ и ставится КАК ВЫБРАННЫЙ В СПИСКЕ, а не как невидимая настройка: она
   * говорит, на чём человек пишет сейчас, что заметно точнее браузерного списка языков, - но с русской
   * раскладкой можно заговорить по-английски. Виден в панели, меняется щелчком, «Auto» рядом.
   *
   * И НИЧЕГО НЕ ДЕЛАЕТ, ЕСЛИ ЗАНЯТО: нажать второй аккорд посреди уже идущей записи или пока читают
   * план значило бы начать писать поверх. */
  useEffect(() => {
    window.__mouseflowSay = (lang?: string) => {
      if (stage.at !== 'writing' || dictation.listening || dictation.recognising) return;
      if (lang && dictation.choices.includes(lang)) dictation.setLang(lang);
      else if (lang) {
        /* Раскладка отдаёт базовый код ("ru"), а в списке может стоять полный ("ru-RU"): это один и тот
         * же выбор, и не узнать его значило бы молча заговорить на другом языке. */
        const near = dictation.choices.find((tag) => tag.split('-')[0] === lang);
        if (near) dictation.setLang(near);
      }
      dictation.start();
    };
    return () => { delete window.__mouseflowSay; };
  }, [stage.at, dictation]);
