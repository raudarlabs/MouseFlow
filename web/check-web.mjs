...
group('каждый живой маршрут - это экран, и у каждого экрана есть маршрут');
{
   ...
   *
   * /panel - четвёртый род, и освобождение ЗАРАБОТАНО, а не объявлено: страница живёт в нативном окне
   * агента размером с подсказку (SPLIT-PLAN §7, шаг 16), где боковая панель заняла бы всё, ради чего это
   * окно есть. Пункта в меню у неё быть не должно - туда не ходят мышью. Ниже проверяется, что она
   * действительно рисуется без обстановки: список исключений, в который можно просто дописать строку, -
   * это способ спрятать экран от проверки. */
  const BARE = ['/sign-in', '/sign-up', '/reset-password', '/admin', '/users', '/models', '/', '/panel'];
  const layout = read('src/shell/AppLayout.tsx');
  check('и /panel освобождён потому, что оболочка его правда не рисует',
    /isPanelPath\(s\.location\.pathname\)/.test(layout)
      && /=== '\/panel'/.test(read('src/shell/AccountProvider.tsx')));
...
group('адрес сильнее выбора');
{
  ...
  /* ОБЩИХ ЭКРАНОВ БОЛЬШЕ НЕТ - шаг 7 закрыт 2026-09-22, и это конец разделения экранов: у каждого адреса
   * ровно один владелец. `'both'` осталось значением типа, а не состоянием приложения: следующий экран,
   * который отвечает обоим, обязан его объявить, а не молча принадлежать тому, кто его первым открыл. */
  check('общих экранов не осталось ни одного',
    SCREENS.filter((s) => s.owner === 'both').length === 0,
    show(SCREENS.filter((s) => s.owner === 'both').map((s) => s.to)));
  check('и Skills теперь называет свой продукт, а не молчит', productAt('/skills') === 'make');
