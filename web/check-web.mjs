...
group('порядок меню закреплён по каждому продукту');
{
  ...
  const ORDER = {
    do: '/create,/activity,/skills,/tests,/dashboard,/team,/gallery',
...
group('адрес сильнее выбора');
{
   ...
   * переключать оболочку под человеком. */
  check('а общий не называет никакой', productAt('/skills') === null && productAt('/dashboard') === null);
