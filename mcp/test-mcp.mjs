...
group('в api/ нет ничего, что не должно быть маршрутом');
{
  ...
  /* queue.js - SPLIT-PLAN §7, шаг 16. Своя дверь по тому же доводу, что у schedules.js: у страницы
   * сессионная кука. А понадобилась потому, что прогон с Create идёт МИМО очереди - браузер сам ведёт
   * цикл, - и это верно для вкладки, которую держат открытой, и неверно для панели, которую закрывают
   * через секунду. Работа в очереди переживает окно. */
...
group('Activity отвечает целиком: идёт, ждёт, было - с действием у каждой строки');
{
  ...
  /* ЖИВЫЕ КАРТОЧКИ - КАК НА SKILLS: в ободке, с зазором, с окном на семь строк и прокруткой. Их две -
   * идущее и ждущее, - и строки в них те же плашки, что у библиотеки. */
  check('две живые карточки в том же ободке, что у библиотеки, и строки - те же плашки',
    (page.match(/rounded-xl border-stroke border bg-surface-card p-4/g) || []).length === 2
  ...
  check('и у каждой окно на семь строк с прокруткой - зазор как у Skills, высота строки своя, измеренная',
    /const LIST_ROW = 2\.75;/.test(page) && /const LIST_GAP = 0\.375;/.test(page)
      ...
      && (page.match(/overflow-y-auto pe-1/g) || []).length === 2);

  /* ЖУРНАЛ - ТАБЛИЦА, и это РАЗВОРОТ прежнего решения, поэтому закреплено и то, чем закрыто возражение.
   *
   * Возражений против таблицы было два: цель растягивалась на всю ширину 1920, и рядом стояли карточки, из
   * которых страница читалась как две разные. Первое закрыто тем, что осталось: цель по-прежнему обрезана
   * по 42rem (пин ниже), лишнее место уходит служебным столбцам. Второе - тем, что экранов в этом
   * приложении четыре, а не восемь, и страница теперь называется журналом.
   *
   * Что таблица даёт поверх плашек и чего в плашке не бывает: ИМЕНА СТОЛБЦОВ. «14:02 · 1m 20s · schedule»
   * - три числа рядом, и ни одно не названо. Шапка при этом липкая: на десятой строке без неё непонятно,
   * какой столбец какой. */
  check('журнал - таблица с назваными столбцами и липкой шапкой',
    /<table className="w-full border-collapse/.test(page)
      && /className="sticky top-0 z-10 bg-surface-card2\/95/.test(page)
      && ['What was asked', 'Outcome', 'Source', 'Started', 'Took'].every((h) => page.includes('>' + h + '</th>')));
  /* Ободок тот же, а внутреннего отступа у него нет: его дают полоса фильтров и таблица, каждая своим. */
  check('и в таком же ободке, но padding внутри дают полоса и таблица',
    /<section className="rounded-xl border-stroke border bg-surface-card">/.test(page));
  /* Отказ подкрашен - журнал открывают, чтобы найти упавшее. Оттенок еле заметный: строка остаётся
   * строкой, а не предупреждением. */
  check('отказавшая строка подкрашена, а не только помечена словом',
    /tone === 'bad' && 'bg-fb-red\/\[0\.055\]'/.test(page));
  /* ВЫГРУЗКА - ТОГО, ЧТО ПОКАЗАНО. Человек с тремя фильтрами просит эти строки; отдать ему всё - это тихо
   * подменить вопрос. И в файле цель ЦЕЛИКОМ: обрезанная делает файл бесполезным ровно для того, зачем его
   * открывают. */
  check('выгрузка - того, что показано, и цель в ней целиком',
    /rowsToCsv\(shown, CSV_COLUMNS\)/.test(page)
      && !/rowsToCsv\(entries/.test(page)
      && /\{ header: 'Asked for', get: \(e\) => \(e\.kind === 'run' \? e\.run\.goal : e\.job\.goal\) \?\? '' \}/.test(page));
  /* RFC 4180, и ПРОВЕРЕНО ИСПОЛНЕНИЕМ, а не поиском по тексту: правильность экранирования - это про
   * значение, которое функция вернула, и никакой regexp по исходнику её не подтверждает. node снимает
   * типы с .ts сам, так что модуль просто импортируется.
   *
   * Цель прогона печатает человек: запятая в ней обычное дело, и одна такая строка без кавычек сдвинула
   * бы все столбцы правее себя - ровно в той строке, из-за которой файл и открыли. */
  {
    const { rowsToCsv, stampedName } = await import('../web/src/lib/csv.ts');
    const cols = [{ header: 'A', get: (r) => r.a }, { header: 'B, count', get: (r) => r.b }];
    const got = rowsToCsv([
      { a: 'plain', b: 1 },
      { a: 'has, comma', b: 2 },
      { a: 'has "quote"', b: 3 },
    ...
    check('запятая в значении берётся в кавычки, а не сдвигает столбцы',
      got.includes('"has, comma",2'), got);
    check('кавычка удваивается', got.includes('"has ""quote""",3'), got);
    check('перевод строки не рвёт строку файла', got.includes('"two\nlines",4'), got);
    check('и шапка экранирована так же, как значения', got.startsWith('A,"B, count"\r\n'), got);
    check('строки разделены CRLF - иначе Excel на Windows читает файл одной строкой',
      got.split('\r\n').length === 6, String(got.split('\r\n').length));
    check('пустое и отсутствующее - пустая ячейка, а не «null»',
      got.endsWith('\r\n,') && !/null|undefined/.test(got), got.slice(-12));
    /* Пустой файл нельзя отличить от неудавшейся выгрузки; файл с одной шапкой - можно. */
    check('без строк остаётся шапка, а не пустой файл', rowsToCsv([], cols) === 'A,"B, count"');
    /* Двоеточий в имени файла быть не может: на Windows это недопустимый знак. */
    check('в имени файла нет двоеточий', !stampedName('x').includes(':'), stampedName('x'));

    const csv = read('../web/src/lib/csv.ts');
    /* BOM - иначе Excel читает UTF-8 в системной кодировке и русские имена превращаются в кашу. И
     * ЭКРАНИРОВАННЫМ, а не самим знаком: он невидим, нулевой ширины, и первым делом попал сюда
     * буквальным символом - строка читалась как `['' + csv]`, то есть как лишняя пустая строка. */
    check('BOM есть, и он экранирован, а не вписан невидимым знаком',
      csv.includes("const BOM = '\\uFEFF';") && csv.includes('[BOM + csv]')
        && !csv.includes('﻿'), 'csv.ts');
  }
...
group('префикс хода кешируется - самое дорогое место в цикле');
{
  ...
  /* ТРЕТЬЯ ОТМЕТКА - НА ЦЕЛИ (2026-09-20). Открывающее сообщение лежит сразу ЗА границей кеша, а за
   * прогон не меняется ни разу: цель, признак готовности, фон и всё приложенное файлом. Оно уходило
   * заново каждый ход по полной цене - и это было единственной причиной, по которой вложения держали на
   * четырёх тысячах знаков. */
  check('цель тоже попадает в кешируемый префикс',
    /function withCachedOpening/.test(vision) && /messages: withCachedOpening\(body[.]messages\)/.test(vision));
  /* И КОПИЕЙ, ПО ПРИЧИНЕ ХУЖЕ ОБЩЕГО МАССИВА: loop.messages уезжает в run_queue.loop между ходами, и
   * отметка, поставленная на месте, сохранилась бы В БАЗЕ - то есть вернулась бы во все будущие запросы
   * этого прогона, где она уже не первая. */
  check('и не мутирует сообщения - они уезжают в run_queue.loop между ходами',
    /\{ [.][.][.]first, content: \[/.test(vision) && !/first[.]content\[.*\][.]cache_control =/.test(vision));
}

/* ------------------------------------------------------------------------------------------------- */

group('потолок цели - одно число, а не четыре совпадающих (вложения, 2026-09-20)');
{
  /* ЧТО ЭТО ЗА ПРОБЛЕМА. Потолок стоял в трёх местах сразу: Create обрезал набранное, очередь клала в
   * user_run.goal первые 4000 знаков, страница столько же в run_queue.loop. Совпадали они по
   * КОММЕНТАРИЮ - attach.ts честно писал, что его число равно числу очереди нарочно, «иначе прогон из
   * Create помнил бы цель целиком, а тот же прогон, поднятый расписанием, - обрезанную, и разница бы
   * нигде не всплыла». Три числа, связанные обещанием, расходятся молча; связанные импортом - нет. */
  const brain = read('../api/_brain.mjs');
  check('число живёт в мозге, и оно одно', /export const GOAL_MAX = \d+;/.test(brain));
  /* ЧЕРЕЗ ОДИН ПЕРЕХОД, И ПРОВЕРЯЕТСЯ ВЕСЬ ПУТЬ (2026-09-20). Сборка цели уехала в api/_attach.mjs -
   * у приложенного появился второй источник, документ из чата (SPLIT-PLAN §7.2). Create теперь читает
   * потолок оттуда, а оттуда - из мозга. Проверять надо обе ссылки: пин, смотрящий только на первую,
   * пропустит тот самый день, когда в середине цепочки заведут своё число. */
  check('и Create больше не объявляет своё, а реэкспортирует его',
    /export \{ GOAL_MAX[^}]*\} from '[.][.]\/[.][.]\/[.][.]\/[.][.]\/api\/_attach[.]mjs'/
      .test(read('../web/src/features/create/attach.ts')));
  check('и посредник его тоже не объявляет, а берёт из мозга',
    /export \{ GOAL_MAX \} from '[.]\/_brain[.]mjs';/.test(read('../api/_attach.mjs'))
      && !/export const GOAL_MAX = \d/.test(read('../api/_attach.mjs')));
  check('очередь сохраняет по нему же',
    /slice\(0, GOAL_MAX\)/.test(read('../api/_mcp-worker.mjs')));
  check('и страница тоже', /slice\(0, GOAL_MAX\)/.test(read('../api/mcp.js')));
  /* НИ ОДНА ИЗ ТРЁХ ДВЕРЕЙ НЕ РЕЖЕТ ЦЕЛЬ ЧИСЛОМ - иначе одна так и обрезает по-старому, а заметить это
   * можно было бы только на файле длиннее четырёх тысяч знаков, то есть в проде.
   *
   * ИМЕННО ЦЕЛЬ, а не «число 4000 в файле»: первая редакция этого пина запрещала `slice(0, 4000)` во всём
   * _mcp-worker.mjs и поймала `body.said` - сводку прогона, у которой свой потолок и свои причины. Пин,
   * запрещающий число, а не место, гонит следующего читателя чинить то, что не сломано. */
  check('ни одна из дверей не режет ЦЕЛЬ зашитым числом',
    !/goal[^;]{0,40}slice\(0, \d+\)/.test(read('../api/_mcp-worker.mjs'))
      && !/goal[^;]{0,40}slice\(0, \d+\)/.test(read('../api/mcp.js')));
  /* ПОЧЕМУ ВЫРОСЛО - сказано там же, где число: без кеша на открывающем сообщении двадцать килобайт
   * платились бы тринадцать раз. Связь названа, чтобы следующий не поднял потолок, не подняв кеш. */
  check('и рядом с числом сказано, чем оплачен его рост',
    /кешируемый префикс/.test(brain) && /_vision[.]mjs/.test(brain));
}
